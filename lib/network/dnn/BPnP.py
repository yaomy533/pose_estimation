...
class BPnP(torch.autograd.Function):
    ...
    def forward(ctx, pts2d, pts3d, K, ini_pose=None):
        ...
        for i in range(bs):
            ...
            if ini_pose is None:
                _, rvec0, T0, _ = cv.solvePnPRansac(objectPoints=pts3d_np, imagePoints=pts2d_i_np, cameraMatrix=K_np,
            ...
            _, rvec, T = cv.solvePnP(objectPoints=pts3d_np, imagePoints=pts2d_i_np, cameraMatrix=K_np, distCoeffs=None,
                                     flags=cv.SOLVEPNP_ITERATIVE, useExtrinsicGuess=True, rvec=rvec0, tvec=T0)
    ...
    def backward(ctx, grad_output):
        ...
        for i in range(bs):
            ...
            pts2d_flat = pts2d[i].clone().contiguous().view(-1).detach().requires_grad_()
            P_6d_flat = P_6d[i].clone().contiguous().view(-1).detach().requires_grad_()
            pts3d_flat = pts3d.clone().contiguous().view(-1).detach().requires_grad_()
...
class BPnPModle(nn.Module):
    def __init__(self):
        super(BPnPModle, self).__init__()

    def forward(self, pts2d, pts3d, K, ini_pose=None):
        return BPnP.apply(pts2d, pts3d, K, ini_pose)
