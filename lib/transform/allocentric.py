...
def apose2epose(allo_r: torch.tensor, allo_t: torch.tensor, eps=1e-7):
    # translation: bsx3
    # rot_allo: bsx3x3
    # Compute rotation between ray to object centroid and optical center ray
    bs = allo_r.size(0)
    cam_ray = torch.tensor([0, 0, 1.0], dtype=allo_t.dtype, device=allo_t.device)
    obj_ray = allo_t / (torch.linalg.norm(allo_t, dim=1, keepdim=True) + eps)
    ...
    rot_allo_to_ego = quat2mat_torch(q_allo_to_ego)
    # Apply quaternion for transformation from allocentric to egocentric.
    ego_rot = torch.matmul(rot_allo_to_ego, allo_r)
    ego_t = torch.matmul(rot_allo_to_ego, allo_t.view(bs, 3, 1)).view(bs, 3)
    return ego_rot, ego_t


def epose2apose(ego_r: torch.tensor, ego_t: torch.tensor, eps=1e-7, rc=False):
    # translation: bsx3
    # rot_allo: bsx3x3
    # Compute rotation between ray to object centroid and optical center ray
    bs = ego_r.size(0)
    cam_ray = torch.tensor([0, 0, 1.0], dtype=ego_t.dtype, device=ego_t.device)
    obj_ray = ego_t / (torch.linalg.norm(ego_t, dim=1, keepdim=True) + eps)
    ...
    rot_allo_to_ego = quat2mat_torch(q_allo_to_ego)
    # Apply quaternion for transformation from allocentric to egocentric.
    rot_ego = torch.matmul(rot_allo_to_ego.permute(0, 2, 1), ego_r)
    trans_ego = torch.matmul(rot_allo_to_ego.permute(0, 2, 1), ego_t.view(bs, 3, 1)).view(bs, 3)
    if rc:
        return rot_ego, trans_ego, rot_allo_to_ego
    else:
        return rot_ego, trans_ego


if __name__ == '__main__':
    cd = torch.randn(64, 1000, 3)
    r = torch.randn(64, 3, 3)
    t = torch.randn(64, 1, 3)
    trans_cd = cd @ r + t
    print(trans_cd.shape)
